...
| discovery_path               | Custom path for the OpenID Connect discovery document. Requires `discovery: true`. Useful when the provider publishes configuration at a non-standard path.              | no       | .well-known/openid-configuration | ".well-known/wallet-openid-configuration"       |
