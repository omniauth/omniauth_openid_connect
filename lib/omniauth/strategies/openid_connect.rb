  ...
  module Strategies
    class OpenIDConnect
      ...
      def callback_phase
        ...
        verify_id_token! if id_token_response?

        discover!
        client.redirect_uri = redirect_uri

        return id_token_callback_phase if response_type == 'id_token'
      ...
      def valid_response_type?
        return true if params.key?(response_type)

        error_attrs = RESPONSE_TYPE_EXCEPTIONS[response_type]
      ...
      def response_type
        @response_type ||= options.response_type.to_s
      end

      def id_token_response?
        Array(response_type).include?('id_token')
      end
